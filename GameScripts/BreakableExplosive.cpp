...
void BreakableExplosive::drawGizmo()
{
    DebugDrawAPI::drawCircle(
        TransformAPI::getGlobalPosition(GameObjectAPI::getTransform(getOwner())),
        Vector3::UnitY,
...
void BreakableExplosive::onBreak()
{
    ...
    const std::vector<GameObject*> objectsInArea = SceneAPI::getObjectsInCircularArea(
        Vector2(explosionCenter.x, explosionCenter.z),
        m_explosionRadius
    );
    ...
    Debug::log(
        "[BreakableExplosive] '%s' exploded dealing %.1f damage in radius %.1f.",
