...
void UIImage::drawUi()
{
    ...
    if (ImGui::BeginDragDropTarget())
    {
        if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("ASSET"))
        {
            ...
            m_texture = nullptr;
            m_textureAsset = std::static_pointer_cast<TextureAsset>(app->getAssetModule()->requestAsset(*data));
            if (m_textureAsset)
            {
...
bool UIImage::deserializeJSON(const rapidjson::Value& componentInfo)
{
    if (componentInfo.HasMember("TextureAssetId"))
    {
        ...
        m_texture = nullptr;
        m_textureAsset = std::static_pointer_cast<TextureAsset>(app->getAssetModule()->requestAsset(m_textureAssetId));
