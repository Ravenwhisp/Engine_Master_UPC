...
Quadtree::Quadtree(const RectangleData& area, UINT depth) : m_bounds(area), m_depth(depth)
{
...
void Quadtree::subdivide()
{
	...
	m_children[0] = new Quadtree(RectangleData(m_bounds.x, m_bounds.y, halfWidth, halfHeight), m_depth + 1); // Top-left
	m_children[1] = new Quadtree(RectangleData(m_bounds.x + halfWidth, m_bounds.y, halfWidth, halfHeight), m_depth + 1); // Top-right
	m_children[2] = new Quadtree(RectangleData(m_bounds.x, m_bounds.y + halfHeight, halfWidth, halfHeight), m_depth + 1); // Bottom-left
	m_children[3] = new Quadtree(RectangleData(m_bounds.x + halfWidth, m_bounds.y + halfHeight, halfWidth, halfHeight), m_depth + 1); // Bottom-right
...
bool Quadtree::isInside(Quadtree& quad, const Vector3& position)
{
	return quad.m_bounds.contains(position);
}
...
bool Quadtree::intersects(const BoundingFrustum& frustum, const RectangleData& rectangle, int minY, int maxY) const
{
	Plane* planes = getFrustumPlanes(frustum);
...
{
	std::vector<RectangleData> quadrants;
	gatherRectangles(quadrants);
	return quadrants;
}
...
void Quadtree::gatherRectangles(std::vector<RectangleData>& outRectangles) const
{
	outRectangles.push_back(m_bounds);

	for (const Quadtree* child : m_children)
	{
		if (child)
		{
			child->gatherRectangles(outRectangles);
		}
	}
}
