...
bool Texture::resize(uint32_t newWidth, uint32_t newHeight)
{
    ...
    app->getResourcesModule()->deferResourceRelease(m_Resource);
    m_Resource.Reset();
...
void Texture::createSRV()
{
    ...
    m_srv = app->getDescriptorsModule()
...
void Texture::createRTV()
{
    ...
    for (uint32_t mip = 0; mip < m_mipCount; ++mip)
    {
        ...
        m_rtv[mip] = app->getDescriptorsModule()
            ->getHeap(D3D12_DESCRIPTOR_HEAP_TYPE_RTV)
            .allocate();
...
void Texture::createDSV()
{
    ...
    m_dsv = app->getDescriptorsModule()
...
void Texture::createUAV()
{
    ...
    for (uint32_t mip = 0; mip < m_mipCount; ++mip)
    {
        ...
        m_uav[mip] = app->getDescriptorsModule()
            ->getHeap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV)
            .allocate();
...
{
    auto* descriptors = app->getDescriptorsModule();
