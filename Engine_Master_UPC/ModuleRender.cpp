...
bool ModuleRender::init()
{
    ...
    m_scenePasses.push_back(std::move(skyBoxPass));
    m_scenePasses.push_back(std::unique_ptr<MeshRendererPass>(m_meshRenderPass));
    m_scenePasses.push_back(std::make_unique<ParticlesPass>(device));

    // Overlay passes draw into the LDR target after post-processing.
    m_overlayPasses.push_back(std::move(debugDrawPass));
    m_overlayPasses.push_back(std::make_unique<UIImagePass>(device));
    m_overlayPasses.push_back(std::make_unique<FontPass>(device));

    m_bloomPass = std::make_unique<BloomPass>(device);
...
void ModuleRender::preRender()
{
...
#ifndef GAME_RELEASE
    {
        ...
        {
            renderToSurface(commandList, *entry.surface, [&](const RenderSurface& surface)
                {
                    if (entry.type == ViewportType::EDITOR)
                    {
                        PERF_RENDER("ModuleRender::RenderEditorScene");
                        renderEditorScene(commandList, surface, entry.width, entry.height);
                    }
                    else
                    {
                        PERF_RENDER("ModuleRender::RenderPlayScene");
                        renderPlayScene(commandList, surface, entry.width, entry.height);
                    }
                });
        }
    }
...
void ModuleRender::renderToSurface(ID3D12GraphicsCommandList4* commandList,RenderSurface& surface,std::function<void(const RenderSurface&)> renderFunc)
{
    // final destination of post-processing + overlays.
...
void ModuleRender::renderScene(ID3D12GraphicsCommandList4* commandList, const RenderCamera& camera, const RenderSurface& surface, D3D12_VIEWPORT viewport, D3D12_RECT  scissorRect, bool renderDebug, RenderViewType viewType)
{
    PERF_RENDER(renderDebug ? "ModuleRender::renderScene(Editor)" : "ModuleRender::renderScene(Game)");

    auto hdrTex = surface.getTexture(RenderSurface::COLOR_1);
    auto ldrTex = surface.getTexture(RenderSurface::COLOR_0);
    auto depthTex = surface.getTexture(RenderSurface::DEPTH_STENCIL);

    const D3D12_CPU_DESCRIPTOR_HANDLE hdrRtv = hdrTex->getRTV(0).cpu;
    const D3D12_CPU_DESCRIPTOR_HANDLE ldrRtv = ldrTex->getRTV(0).cpu;
    const D3D12_CPU_DESCRIPTOR_HANDLE dsv = depthTex->getDSV().cpu;
    ...
    transitionResource(commandList, hdrTex->getD3D12Resource(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);

    {
        PERF_RENDER("ModuleRender::renderScene::Background");
        renderBackground(commandList, hdrRtv, dsv, viewport, scissorRect);
    }

    {
        PERF_RENDER("ModuleRender::renderScene::ScenePasses");
    ...
    transitionResource(commandList, hdrTex->getD3D12Resource(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);

    //blur the bright areas of the HDR scene.
    const bool bloomEnabled = app->getModuleScene()->getScene()->getPostProcessSettings().bloomEnabled;
    if (bloomEnabled && m_bloomPass)
    {
        PERF_RENDER("ModuleRender::renderScene::Bloom");
        m_bloomPass->prepare(ctx);
        m_bloomPass->apply(commandList, hdrTex->getSRV().gpu);
        m_postProcessPass->setBloomSource(m_bloomPass->getBloomSRV(), true);
    }

    // Resolve HDR -> LDR into COLOR_0.
    {
        PERF_RENDER("ModuleRender::renderScene::PostProcess");
        if (m_postProcessPass)
        {
            m_postProcessPass->prepare(ctx);
            m_postProcessPass->apply(commandList, hdrTex->getSRV().gpu, ldrRtv, viewport, scissorRect);
        }
    }

    //Overlays draw on top of the resolved image.
    {
        PERF_RENDER("ModuleRender::renderScene::OverlayPasses");
        commandList->OMSetRenderTargets(1, &ldrRtv, FALSE, &dsv);
...
void ModuleRender::renderEditorScene(ID3D12GraphicsCommandList4* commandList, const RenderSurface& surface, float width, float height)
{
    D3D12_VIEWPORT viewport = { 0, 0, width, height, 0, 1 };
    D3D12_RECT     scissorRect = { 0, 0, static_cast<LONG>(width), static_cast<LONG>(height) };

    renderScene(commandList, getEditorCamera(), surface, viewport, scissorRect, /*debug=*/true, RenderViewType::Editor);
}

void ModuleRender::renderPlayScene(ID3D12GraphicsCommandList4* commandList, const RenderSurface& surface, float width, float height)
{
    ...
    renderScene(commandList, camera, surface, viewport, scissorRect, m_moduleGameView->getShowDebugWindow(), RenderViewType::Game);
}

void ModuleRender::renderGameToBackbuffer(ID3D12GraphicsCommandList4* commandList, const RenderSurface& surface, D3D12_VIEWPORT viewport, D3D12_RECT scissorRect)
{
    const RenderCamera camera = getGameCamera();
    if (!camera.valid) return;
