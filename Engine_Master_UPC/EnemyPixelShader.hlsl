...
cbuffer PlayerData : register(b5)
{
    ...
    float3 dissolveEffectRimColor;
    float dissolveEffectRimIntensity;
}
...
float4 main(float3 worldPos : POSITION, float3 normal : NORMAL, float3 tangent : TANGENT, float2 coord : TEXCOORD, float4 position : SV_POSITION) : SV_TARGET
{
    ...
    int dissolveEffect = CalculateDissolveEffect();
    if (dissolveEffect == 1)
    {
        return float4(dissolveEffectRimColor, 1.0f);
    }
    else if (dissolveEffect == 2)
    {
        return float4(0);
    }
