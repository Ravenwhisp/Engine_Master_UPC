...
private:
    void renderToSurface( ID3D12GraphicsCommandList4* commandList, RenderSurface& surface, std::function<void(RenderSurface& surface)> renderFunc);

    // Scene rendering
    void renderScene( ID3D12GraphicsCommandList4* commandList, const RenderCamera& camera, RenderSurface& surface, D3D12_VIEWPORT viewport, D3D12_RECT scissorRect, bool renderDebug, RenderViewType viewType);
    ...
    void renderEditorScene(ID3D12GraphicsCommandList4* commandList, RenderSurface& surface, float width, float height);
    void renderPlayScene(ID3D12GraphicsCommandList4* commandList, RenderSurface& surface, float width, float height);
    void renderGameToBackbuffer( ID3D12GraphicsCommandList4* commandList, RenderSurface& surface, D3D12_VIEWPORT viewport, D3D12_RECT     scissorRect);
