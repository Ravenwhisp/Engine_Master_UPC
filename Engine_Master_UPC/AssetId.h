...
struct AssetId {
    ...
    AssetId() = default;
    explicit AssetId(UID uid): m_uid(uid) {}

    AssetId(UID uid, MD5Hash libId) : m_uid(uid), m_libId(std::move(libId)) {}

    AssetId(UID uid, MD5Hash libId, AssetType type) : m_uid(uid), m_libId(std::move(libId)), m_type(type) {}
