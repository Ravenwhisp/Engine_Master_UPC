...
EnemyPass::EnemyPass(ComPtr<ID3D12Device4> device)
{
    m_device = device;
...
void EnemyPass::createRootSignature()
{
    ...
    rootParams[5].InitAsConstantBufferView(5, 0, D3D12_SHADER_VISIBILITY_PIXEL); //Enemy data
...
void EnemyPass::createPipelineState()
{
    ComPtr<ID3DBlob> vertexShaderBlob;
    ThrowIfFailed(D3DReadFileToBlob(L"EnemyVertexShader.cso", &vertexShaderBlob));
...
void EnemyPass::prepare(const RenderContext& ctx)
{
    m_view = &ctx.view;
    ...
    m_meshRenderers = app->getModuleScene()->getVisibleForwardMeshRenderers(RenderMode::ENEMY);
...
void EnemyPass::apply(ID3D12GraphicsCommandList4* commandList)
{
    BEGIN_EVENT(commandList, "EnemyPass");
    ...
    commandList->OMSetStencilRef(static_cast<UINT>(RenderMode::ENEMY));
...
void EnemyPass::renderMeshRenderer(ID3D12GraphicsCommandList4* commandList, MeshRenderer* renderer)
{
    ...
    EnemyRenderBufferComponent* enemyRenderBufferComponent = static_cast<EnemyRenderBufferComponent*>(owner->GetComponent(ComponentType::ENEMY_RENDER_BUFFER));
    EnemyRenderBufferComponent::EnemyRenderBuffer enemyRenderBuffer{};
    if (enemyRenderBufferComponent)
    {
        enemyRenderBuffer.damageHighlight = enemyRenderBufferComponent->getDamageHighlightIntensity();
        enemyRenderBuffer.damageHighlightData = enemyRenderBufferComponent->getDamageHighlightData();

        enemyRenderBuffer.dissolveEffect = enemyRenderBufferComponent->getDissolveEffectIntensity();
        enemyRenderBuffer.dissolveEffectData = enemyRenderBufferComponent->getDissolveEffectData();
    }
    commandList->SetGraphicsRootConstantBufferView(5, app->getModuleRender()->allocateInRingBuffer(&enemyRenderBuffer, sizeof(EnemyRenderBufferComponent::EnemyRenderBuffer)));
...
GPULightsConstantBuffer EnemyPass::packLightsForGPU(const std::vector<LightComponent*>& lights, const Vector3& ambientColor, float ambientIntensity) const
{
